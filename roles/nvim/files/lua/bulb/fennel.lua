    ...
    or function(...)
        ...
        local function completer(env, scope, text)
            ...
            local function add_partials(input, tbl, prefix)
                local scope_first_3f = (
                    (tbl == env) or (tbl == env.___replLocals___)
                )
            ...
            do
                ...
                for name, f in pairs(commands) do
                    local val_16_auto = ("  ,%s - %s"):format(
                        name,
                        (
                            (compiler.metadata):get(f, "fnl/docstring")
                            or "undocumented"
                        )
                    )
                    if nil ~= val_16_auto then
        ...
        commands.complete =
            function(env, read, on_values, on_error, scope, chars)
            ...
            for _, path in ipairs(apropos ".*") do
                ...
                do
                    ...
                    if "function" == type(tgt) then
                        local _666_ = (compiler.metadata):get(
                            tgt,
                            "fnl/docstring"
                        )
                        if nil ~= _666_ then
        ...
        commands["apropos-doc"] =
            function(_env, read, on_values, on_error, _scope)
        ...
        commands["apropos-show-docs"] =
            function(_env, read, on_values, on_error)
        ...
        commands.find = function(env, read, on_values, on_error, scope)
            local function _679_(_241)
                ...
                if
                    (_G.type(_680_) == "table")
                    and ((_680_).what == "Lua")
                    and (nil ~= (_680_).source)
                    and (nil ~= (_680_).linedefined)
                    and (nil ~= (_680_).short_src)
                then
                    ...
                    return on_values {
                        string.format("%s:%s", src, (fnlsrc or line)),
                    }
        ...
        commands.doc = function(env, read, on_values, on_error, scope)
            local function _690_(_241)
                ...
                if ok_3f then
                    ...
                    return on_error(
                        "Repl",
                        "Could not resolve value for docstring lookup"
                    )
                end
            end
        ...
        commands.compile = function(env, read, on_values, on_error, scope)
            local function _693_(_241)
                ...
                local ok_3f, result = pcall(compiler.compile, _241, {
                    env = env,
                    scope = scope,
                ...
                if ok_3f then
                    ...
                    return on_error(
                        "Repl",
                        ("Error compiling expression: " .. result)
                    )
                end
            end
        ...
        local function repl(_3foptions)
            ...
            local readline = (
                should_use_readline_3f(opts)
                and try_readline_21(opts, pcall(require, "readline"))
            )
            local env =
                specials["wrap-env"]((opts.env or rawget(_G, "_ENV") or _G))
                    ...
                    if not_eof_3f then
                        do
                            ...
                            elseif (_718_ == true) and (nil ~= _719_) then
                                ...
                                if save_locals_3f then
                                    src0 =
                                        splice_save_locals(env, src, opts.scope)
                                else
                                    src0 = src
                                end
                                local _723_, _724_ =
                                    pcall(specials["load-code"], src0, env)
...
package.preload["fennel.specials"] = package.preload["fennel.specials"]
    or function(...)
        ...
        local function wrap_env(env)
            ...
            return setmetatable(
                {},
                { __index = _417_, __newindex = _419_, __pairs = _421_ }
            )
        end
        ...
        local function doc_2a(tgt, name)
            if not tgt then
                ...
                local docstring = (((compiler.metadata):get(
                    tgt,
                    "fnl/docstring"
        ...
        SPECIALS["do"] = function(
            ...
            local function compile_body(
                ...
                if len < start then
                    ...
                    for i = start, len do
                        ...
                        local subexprs =
                            compiler.compile1(ast[i], sub_scope, chunk, subopts)
                        if i ~= len then
            ...
            elseif opts.nval then
                ...
                for i = 1, opts.nval do
                    local s = (
                        (_3fpre_syms and (_3fpre_syms)[i])
                        or compiler.gensym(scope)
                    )
                    do
                    end
                    (syms)[i] = s
                ...
                compiler.emit(
                    parent,
                    string.format("local %s", outer_target),
                    ast
                )
                compiler.emit(parent, "do", ast)
        ...
        local function compile_named_fn(
            ...
            compiler.emit(
                parent,
                string.format(_455_, fn_name, table.concat(arg_name_list, ", ")),
                ast
            )
            compiler.emit(parent, f_chunk, ast)
            compiler.emit(parent, "end", ast)
            set_fn_metadata(
                f_metadata["fnl/arglist"],
                f_metadata["fnl/docstring"],
                parent,
                fn_name
            )
        ...
        SPECIALS.fn = function(ast, scope, parent)
            ...
            local fn_name, local_3f, index =
                get_fn_name(ast, scope, fn_sym, multi)
            ...
            local function get_arg_name(arg)
                ...
                elseif utils["table?"](arg) then
                    ...
                    local declared =
                        compiler["declare-local"](raw, {}, f_scope, ast)
                    ...
                    return compiler.assert(
                        false,
                        ("expected symbol for function parameter: %s"):format(
                            tostring(arg)
                        ),
                        ast[index]
                    )
            ...
            local f_metadata, index0 =
                get_function_metadata(ast, arg_list, index)
            if fn_name then
                return compile_named_fn(
                    ast,
                    f_scope,
        ...
        local function dot(ast, scope, parent)
            ...
            if len == 2 then
                ...
                for i = 3, len do
                    ...
                    if
                        utils["string?"](index)
                        and utils["valid-lua-identifier?"](index)
                    then
                        table.insert(indices, ("." .. index))
                    else
                        local _let_476_ = compiler.compile1(
                            index,
                            scope,
                            parent,
                            { nval = 1 }
        ...
        SPECIALS.tset = function(ast, scope, parent)
            ...
            for i = 3, (#ast - 1) do
                local _let_487_ =
                    compiler.compile1(ast[i], scope, parent, { nval = 1 })
                local key = _let_487_[1]
                table.insert(keys, tostring(key))
            end
            local value = (compiler.compile1(
                ast[#ast],
                scope,
                parent,
                { nval = 1 }
        ...
        local function if_2a(ast, scope, parent, opts)
            ...
            local wrapper, inner_tail, inner_target, target_exprs =
                calculate_target(scope, opts)
            local body_opts =
                { nval = opts.nval, tail = inner_tail, target = inner_target }
            ...
            for i = 2, (#ast - 1), 2 do
                local condchunk = {}
                local res =
                    compiler.compile1(ast[i], do_scope, condchunk, { nval = 1 })
                local cond = res[1]
            ...
            if wrapper == "iife" then
                ...
                compiler.emit(parent, "end", ast)
                return utils.expr(
                    ("%s(%s)"):format(tostring(s), iifeargs),
                    "statement"
                )
        ...
        local function remove_until_condition(bindings)
            ...
            if
                (utils["sym?"](last_item) and (tostring(last_item) == "&until"))
                or ("until" == last_item)
            then
        ...
        local function compile_until(condition, scope, chunk)
            if condition then
                local _let_496_ =
                    compiler.compile1(condition, scope, chunk, { nval = 1 })
        ...
        local function while_2a(ast, scope, parent)
            local len1 = #parent
            local condition = (compiler.compile1(
                ast[2],
                scope,
                parent,
                { nval = 1 }
        ...
        local function method_call(ast, scope, parent)
            ...
            for i = 4, #ast do
                ...
                subexprs =
                    compiler.compile1(ast[i], scope, parent, { nval = _503_ })
                utils.map(subexprs, tostring, args)
            end
            if
                utils["string?"](ast[3])
                and utils["valid-lua-identifier?"](ast[3])
            then
        ...
        local function arithmetic_special(
            ...
            for i = 2, len do
                local subast =
                    maybe_short_circuit_protect(ast[i], i, name, scope)
            ...
            if #ast == 1 then
                ...
                if #operands == 1 then
                    ...
                    if utils.root.options.useBitLib then
                        ...
                        return (
                            "("
                            .. table.concat(operands, padded_native_name)
                            .. ")"
                        )
                    end
                end
        ...
        local function define_bitop_special(
            name,
            zero_arity,
            unary_prefix,
            native
        ...
        local function native_comparator(op, _538_, scope, parent)
            ...
            local _let_540_ =
                compiler.compile1(lhs_ast, scope, parent, { nval = 1 })
            local lhs = _let_540_[1]
            local _let_541_ =
                compiler.compile1(rhs_ast, scope, parent, { nval = 1 })
        ...
        local function double_eval_protected_comparator(
            ...
            for i = 2, #ast do
                ...
                table.insert(
                    vals,
                    tostring(
                        (compiler.compile1(ast[i], scope, parent, { nval = 1 }))[1]
                    )
                )
            end
        ...
        local function make_compiler_env(ast, scope, parent, _3fopts)
            ...
            do
                ...
                if
                    (_G.type(_549_) == "table")
                    and ((_549_)["compiler-env"] == "strict")
                then
                    provided = safe_compiler_env()
                elseif
                    (_G.type(_549_) == "table") and (nil ~= (_549_).compilerEnv)
                then
                    local compilerEnv = (_549_).compilerEnv
                    provided = compilerEnv
                elseif
                    (_G.type(_549_) == "table")
                    and (nil ~= (_549_)["compiler-env"])
                then
        ...
        local function search_module(modulename, _3fpathstring)
            ...
            local fullpath = (
                (_3fpathstring or utils["fennel-module"].path)
                .. pkg_config.pathsep
            )
            local function try_path(path)
                local filename =
                    path:gsub(escapepat(pkg_config.pathmark), no_dot_module)
                local filename2 =
                    path:gsub(escapepat(pkg_config.pathmark), modulename)
            ...
            local function find_in_path(start, _3ftried_paths)
                ...
                elseif true then
                    ...
                    local function _566_()
                        local tried_paths =
                            table.concat((_3ftried_paths or {}), "\n\t")
        ...
        local function dofile_with_searcher(
            fennel_macro_searcher,
            filename,
            opts,
            ...
        )
        ...
        local function fennel_macro_searcher(module_name)
            ...
            local _578_ =
                search_module(module_name, utils["fennel-module"]["macro-path"])
            if nil ~= _578_ then
            ...
            if nil ~= _591_ then
                ...
                do
                    ...
                    code = close_handlers_8_auto(
                        _G.xpcall(
                            _593_,
                            (package.loaded.fennel or debug).traceback
                        )
                    )
                end
        ...
        local function _601_(modname)
            ...
            return (
                macro_loaded[modname]
                or sandbox_fennel_module(modname)
                or _602_()
            )
        end
        safe_require = _601_
        ...
        local function include_path(ast, opts, path, mod, fennel_3f)
            ...
            do
                ...
                src = close_handlers_8_auto(
                    _G.xpcall(_610_, (package.loaded.fennel or debug).traceback)
                )
            end
        ...
        SPECIALS.include = function(ast, scope, parent, opts)
            ...
            do
                local _613_, _614_ =
                    pcall(resolve_module_name, ast, scope, parent, opts)
            ...
            if (modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34) then
                ...
                res = (
                    (
                        utils["member?"](
                            mod,
                            (utils.root.options.skipInclude or {})
                        )
                        and opts.fallback(modexpr, true)
                    )
                    or include_circular_fallback(
                        mod,
                        modexpr,
                        opts.fallback,
                        ast
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"]
    or function(...)
        ...
        local function assert_msg(ast, msg)
            ...
            local target =
                tostring((utils["sym?"](ast_tbl[1]) or ast_tbl[1] or "()"))
            return string.format(
        ...
        local function global_allowed_3f(name)
            return (
                not allowed_globals or utils["member?"](name, allowed_globals)
            )
        end
        ...
        local function check_binding_valid(symbol, scope, ast, _3fopts)
            ...
            assert_compile(
                not (
                        scope.specials[name]
                        or (not macro_3f and scope.macros[name])
                    ),
                ("local %s was overshadowed by a special form or macro"):format(
                    name
                ),
                ast
            )
        ...
        local function symbol_to_expression(symbol, scope, _3freference_3f)
            ...
            local name0 = (
                hashfn_arg_name(name, multi_sym_parts, scope) or name
            )
            ...
            assert_compile(
                ...
                "tried to reference a special form at runtime",
                symbol
            )
            assert_compile(
                (
                    ...
                    for _, subchunk in ipairs(chunk) do
                        if subchunk.leaf or (0 < #subchunk) then
                            ...
                            if file == source.filename then
                                last_line0 =
                                    math.max(last_line0, (source.line or 0))
                            else
                            end
                            last_line0 =
                                flatten(subchunk, out, last_line0, file)
                        else
                        end
                    end
                end
            ...
            if chunk.leaf then
                ...
                local function parter(c)
                    if c.leaf or (0 < #c) then
                        local sub =
                            flatten_chunk(file_sourcemap, c, tab0, (depth + 1))
                        if 0 < depth then
        ...
        local function flatten(chunk, options)
            ...
            if options.correlate then
                ...
                local src =
                    flatten_chunk(file_sourcemap, chunk0, options.indent, 0)
        ...
        local function make_metadata()
            ...
            return setmetatable({}, {
                __index = { get = _302_, set = _304_, setall = _305_ },
                __mode = "k",
            })
        end
        ...
        local function keep_side_effects(exprs, chunk, start, ast)
            ...
            for j = start0, #exprs do
                ...
                if (se.type == "expression") and (se[1] ~= "nil") then
                    emit(
                        chunk,
                        string.format("do local _ = %s end", tostring(se)),
                        ast
                    )
        ...
        local function handle_compile_opts(exprs, parent, opts, ast)
            ...
            if opts.target then
                ...
                emit(
                    parent,
                    string.format("%s = %s", opts.target, _313_()),
                    ast
                )
            else
            end
        ...
        local function propagate_trace_info(_322_, _index, node)
            ...
            do
                ...
                if ("table" == type(node)) and (filename ~= src.filename) then
                    src.filename, src.line, src["from-macro?"] =
                        filename, line, true
        ...
        local function compile_special(ast, scope, parent, opts, special)
            local exprs = (
                special(ast, scope, parent, opts)
                or utils.expr("nil", "literal")
            )
            local exprs0
        ...
        local function compile_call(ast, scope, parent, opts, compile1)
            ...
            local special = (
                utils["sym?"](first) and scope.specials[tostring(first)]
            )
            assert_compile(
                (0 < len),
            ...
            elseif
                multi_sym_parts and multi_sym_parts["multi-sym-method-call"]
            then
        ...
        local function compile_sym(ast, scope, parent, opts)
            ...
            assert_compile(
                not (
                        multi_sym_parts
                        and multi_sym_parts["multi-sym-method-call"]
                    ),
        ...
        local function compile_table(ast, scope, parent, opts, compile1)
            ...
            local function write_other_values(k)
                ...
                then
                    if
                        (type(k) == "string")
                        and utils["valid-lua-identifier?"](k)
                    then
                        return { k, k }
                    else
                        local _let_353_ =
                            compile1(k, scope, parent, { nval = 1 })
            ...
            do
                ...
                local function _359_(_357_)
                    ...
                    local _let_360_ =
                        compile1(ast[k2], scope, parent, { nval = 1 })
                    local v = _let_360_[1]
        ...
        local function destructure(to, from, ast, scope, parent, opts)
            ...
            local function destructure_kv_rest(
                s,
                v,
                left,
                excluded_keys,
                ...
                do
                    ...
                    for _, k in ipairs(excluded_keys) do
                        local val_16_auto =
                            string.format("[%s] = true", serialize_string(k))
                        if nil ~= val_16_auto then
                            i_15_auto = (i_15_auto + 1)
                            do
                            end
                ...
                for k, v in utils.stablepairs(left) do
                    ...
                    then
                        ...
                        elseif utils["sym?"](k) and (tostring(k) == "&as") then
                            destructure_sym(
                                v,
                                { utils.expr(tostring(s)) },
                                left
                            )
                        elseif
                            utils["sequence?"](left) and (tostring(v) == "&as")
                        then
                            local _, next_sym, trailing =
                                select(k, unpack(left))
                            assert_compile(
                                (nil == trailing),
            ...
            local function destructure_values(left, up1, top_3f, destructure1)
                ...
                assert_compile(
                    top_3f,
                    "can't nest multi-value destructuring",
                    left
                )
        ...
        local function require_include(ast, scope, parent, opts)
            opts.fallback = function(e, no_warn)
                if not no_warn and ("literal" == e.type) then
                    utils.warn(
                        (
                            "include module not found, falling back to require: %s"
                        ):format(tostring(e))
                    )
                else
                end
        ...
        local function traceback_frame(info)
            ...
            elseif info.what == "C" then
                ...
                if remap and remap[info.currentline] then
                    if
                        (remap[info.currentline][1] or "unknown") ~= "unknown"
                    then
                        info.short_src =
        ...
        local function traceback(_3fmsg, _3fstart)
            ...
            if
                (msg:find "^Compile error" or msg:find "^Parse error")
            ...
            then
                ...
                local lines = {}
                if
                    msg:find ":%d+: Compile error"
                    or msg:find ":%d+: Parse error"
                then
    ...
    or function(...)
        ...
        local function read_line(filename, line, _3fsource)
            if _3fsource then
                ...
                return close_handlers_8_auto(
                    _G.xpcall(_178_, (package.loaded.fennel or debug).traceback)
                )
            end
        end
        ...
        local function highlight_line(codeline, col, _3fendcol, opts)
            ...
            local _let_183_ = (error_pinpoint or { "[7m", "[0m" })
        ...
        local function friendly_msg(msg, _184_, source, opts)
            ...
            if ok and codeline then
                if col then
                    table.insert(
                        out,
                        highlight_line(codeline, col, endcol, opts)
                    )
                else
...
package.preload["fennel.parser"] = package.preload["fennel.parser"]
    or function(...)
        ...
        local function char_starter_3f(b)
            return (
                (function(_199_, _200_, _201_)
                    return (_199_ < _200_) and (_200_ < _201_)
                end)(1, b, 127)
                or (function(_202_, _203_, _204_)
                    return (_202_ < _203_) and (_203_ < _204_)
                end)(192, b, 247)
            )
        end
        local function parser_fn(getbyte, filename, _205_)
            ...
            local function whitespace_3f(b)
                ...
                return (
                    (b == 32)
                    or (function(_212_, _213_, _214_)
                        return (_212_ <= _213_) and (_213_ <= _214_)
                    end)(9, b, 13)
                    or _216_()
                )
            end
            ...
            local function parse_stream()
                ...
                local function dispatch(v)
                    ...
                    if _220_ == nil then
                        retval, done_3f, whitespace_since_dispatch =
                            v, true, false
                        return nil
                    elseif
                        (_G.type(_220_) == "table") and (nil ~= (_220_).prefix)
                    then
                ...
                local function close_list(list)
                    return dispatch(
                        setmetatable(list, getmetatable(utils.list()))
                    )
                end
                ...
                local function close_table(b)
                    ...
                    if top == nil then
                        parse_error(
                            ("unexpected closing delimiter " .. string.char(b))
                        )
                    else
                    end
                ...
                local function parse_string()
                    ...
                    local _244_ = (rawget(_G, "loadstring") or load)(
                        ("return " .. formatted)
                    )
                    if nil ~= _244_ then
                        local load_fn = _244_
                ...
                local function check_malformed_sym(rawstr)
                    ...
                    elseif rawstr:match ":.+[%.:]" then
                        return parse_error(
                            (
                                "method must be last component of multisym: "
                                .. rawstr
                            ),
                            col_adjust ":.+[%.:]"
                        )
                ...
                local function parse_sym(b)
                    ...
                    local rawstr =
                        string.char(unpack(parse_sym_loop({ b }, getb())))
                    ...
                    elseif not parse_number(rawstr) then
                        return dispatch(
                            utils.sym(check_malformed_sym(rawstr), source0)
                        )
                    else
                        return nil
                    end
        ...
        local function parser(stream_or_string, _3ffilename, _3foptions)
            ...
            if "string" == type(stream_or_string) then
                return parser_fn(
                    string_stream(stream_or_string),
                    filename,
                    options
                )
            else
    ...
    or function(...)
        ...
        local function concat_table_lines(
            ...
            then
                ...
                return (
                    open
                    .. table.concat(elements, indent_str)
                    .. _34_()
                    .. close
                )
            else
                ...
                do
                    ...
                    for _, _40_ in ipairs(kv) do
                        ...
                        do
                            ...
                            local v0 =
                                pp(v, options, (indent0 + slength(k0) + 1))
                            multiline_3f = (
                                multiline_3f
                                or k0:find "\n"
                                or v0:find "\n"
        ...
        local function pp_metamethod(t, metamethod, options, indent)
            if options.depth <= options.level then
                ...
                _ = nil
                local lines, force_multi_line_3f =
        ...
        local function utf8_escape(str)
            local function validate_utf8(str0, index)
                ...
                if
                    ...
                    and not (function(_82_, _83_, _84_)
            ...
            while index <= #str do
                local nexti = (
                    string.find(str, "[\128-\255]", index) or (#str + 1)
                )
                local len = validate_utf8(str, nexti)
                table.insert(
                    output,
                    string.sub(str, index, (nexti + (len or 0) + -1))
                )
                if not len and (nexti <= #str) then
                    table.insert(
                        output,
                        string.format("\\%03d", string.byte(str, nexti))
                    )
                else
                end
                if len then
...
package.preload["fennel.utils"] = package.preload["fennel.utils"]
    or function(...)
        ...
        local function sym_3d(a, b)
            return (
                (deref(a) == deref(b)) and (getmetatable(a) == getmetatable(b))
            )
        end
...
mod.install = function(_3fopts)
    table.insert(
        ...
        specials["make-searcher"](_3fopts)
    )
    return mod
end
...
do
  ...
  {:-> ->*
   ...
   :match-try match-try*}
    ...
    do
        local _743_ =
            specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
        do
        end
        (_743_)["utils"] = utils
        _743_["fennel"] = mod
